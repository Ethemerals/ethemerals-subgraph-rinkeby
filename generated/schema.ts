...
export class Account2 extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save Account2 entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save Account2 entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("Account2", id.toString(), this);
    }
  }

  static load(id: string): Account2 | null {
    return changetype<Account2 | null>(store.get("Account2", id));
  }
